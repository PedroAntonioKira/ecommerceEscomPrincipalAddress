...
import (
	...
	"github.com/PedroAntonioKira/ecommerceEscomPrincipalAddress/domain/use_cases"
...
func ProcesoAddress(body string, path string, method string, user string, id int, request events.APIGatewayProxyRequest) (int, string) {
	...
	switch method {
	case "POST":
		fmt.Println("Si entramos A POST de Address")
		return use_cases.AddAddressUC(body, user)
